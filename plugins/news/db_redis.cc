...
// 用于测试
void DbRedis::AppendTestData(NetBase *news_kv) {
  if (NULL == news_kv) {
    return;
  }
  std::string content = "突发性利空包括大盘和上市公司的两方面。这种“突发性”往往许多机构都没有思想和资金等方面的准备，常常是以股价的下跌来回应，而实力强大的庄家则有能力应付和化解各种不利因素，不会轻易随波逐流，它在利空的情况下，可能会以横盘，甚至逆市上扬的走势来表现";
  news_kv->SetString(L"detail", content); 
  news_kv->SetBigInteger(L"transmit_count", 0);
  news_kv->SetString(L"imgsrc", "http://i.thsi.cn/images/master/v2/head-name.png");
  news_kv->SetString(L"indus", "电子信息");
  news_kv->SetString(L"sect", "机顶盒,彩票,虚拟现实,中盘,物联网,谷歌眼镜,智能家居,预盈预增,电子支付,网络游戏");
  news_kv->SetString(L"stock", "300028");
}
